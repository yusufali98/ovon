...
class PointNavSemanticResNetCLIPNet(Net):
    def __init__(
        self,
        observation_space: spaces.Dict,
        action_space,
...
class SemanticResNetCLIPEncoder(nn.Module):
    def __init__(
        self,
        observation_space: spaces.Dict,
        ...
        if not self.is_blind:
            ...
            if "none" in backbone_type:
                ...
                if self.clip_model == "ViT-B/16":
                    self.output_shape = (512 + depth_size + semantic_size,)
                else:
                    self.output_shape = (2048 + depth_size + semantic_size,)
...
class ResNet18SemanticEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        from habitat_baselines.rl.ddppo.policy.resnet import resnet18 as resnet18_semantic
        from habitat_baselines.rl.ddppo.policy.running_mean_and_var import RunningMeanAndVar

        self.running_mean_and_var = RunningMeanAndVar(1)        # NOTE: hardcoded 1 for single obj segmentation mask channel

        self.backbone = resnet18_semantic(in_channels=1,    # NOTE: hardcoded 1 for single obj segmentation mask channel
                                                base_planes=32, 
                                                ngroups=32)

        final_spatial_h = int(np.ceil(224 * self.backbone.final_spatial_compress))      # NOTE: hardcoded 224 for image height
        final_spatial_w = int(np.ceil(224 * self.backbone.final_spatial_compress))      # NOTE: hardcoded 224 for image width
        ...
        self.visual_fc = nn.Sequential(
                                        nn.Flatten(),
                                        nn.Linear(
                                            np.prod(self.sem_output_shape), 512         # NOTE: hardcoded semantic feats hidden size to 512
                                        ),
                                        nn.ReLU(True),
                                    )
